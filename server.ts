...
function gate(ctx: Context): GateResult {
  ...
  if (chatType === 'channel') {
    ...
    if (!policy) {
      process.stderr.write(`[gate] channel_post DROP: channelId=${channelId} not in channels config\n`)
      return { action: 'drop' }
    }
    process.stderr.write(`[gate] channel_post DELIVER: channelId=${channelId}\n`)
...
bot.on('channel_post:photo', async ctx => {
  ...
  await handleInbound(ctx, caption, async () => {
    ...
    try {
      ...
      process.stderr.write(`telegram channel: channel_post photo download failed: ${err}\n`)
      return undefined
    }
  })
})
...
bot.on('channel_post:sticker', async ctx => {
  const sticker = ctx.channelPost.sticker
  const emoji = sticker.emoji ? ` ${sticker.emoji}` : ''
  await handleInbound(ctx, `(sticker${emoji})`, undefined, {
    kind: 'sticker',
    file_id: sticker.file_id,
...
async function handleInbound(
  ...
  // channel_post has no `from` (anonymous channel sender)
  const from = ctx.from ?? { id: Number(ctx.chat!.id), is_bot: false, first_name: 'channel' }
  const chat_id = String(ctx.chat!.id)
  const msgId = (ctx.message ?? ctx.channelPost)?.message_id
  ...
  // Print to stderr so the message appears in the Claude Code terminal.
  process.stderr.write(`[channel inbound] chat=${chat_id} user=${from.username ?? from.id} text=${text}\n`)
  ...
  mcp.notification({
    ...
    params: {
      ...
      meta: {
        ...
        ts: new Date(((ctx.message ?? ctx.channelPost)?.date ?? 0) * 1000).toISOString(),
